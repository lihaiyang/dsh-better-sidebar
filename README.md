...
- **🗃️ 悬浮文件管理器**：右上角按钮或 `Ctrl/Cmd+Shift+E` 随时唤出，独立浮窗可拖拽/缩放/最大化；打开任意目录、路径栏导航、大小/修改时间排序、新建/重命名/删除，并在窗内预览文本/图片/PDF/Office 等所有文件类型（半屏可拖拽比例 + 全屏预览，全局信任，不创建侧边栏 Tab）
...
- **📊 运行状态 / Telemetry**：实时上下文压力（token / 窗口占比）、LLM 输入输出/缓存 token、速度与首 token 延迟、技能目录与模型调用开关（由原 dsh-sidebar-telemetry 合并内置）
...
- **🔌 服务化**：暴露 `ctx.betterSidebar` 服务，其他插件可注册侧边栏 tab 与文件预览器（内置 8 tab + 9 viewer 也走同一服务，详见 [AGENTS.md](./AGENTS.md)）
