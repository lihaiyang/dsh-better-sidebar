...
import { applyTelemetry } from './telemetry.ts'
...
/** Services required before mounting: the webserver routes, the session store, the loader's connection row, the tool registry, the skill registry, and the live-agent registry (scope resolution). */
export const inject = ['webServer', 'sessions', 'loader', 'tools', 'skills', 'agents']
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  // Merged telemetry feature: host routes for the skill catalog / detail /
  // model-invocation toggle (fenced under /telemetry/api).
  applyTelemetry(ctx)
