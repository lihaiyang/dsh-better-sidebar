...
- **🗃️ Floating File Manager**: summon anytime with the top-right button or `Ctrl/Cmd+Shift+E`; independent draggable/resizable/maximizable window; opens any directory, path-bar navigation, sort by size/mtime, new/rename/delete, and in-window preview for all file types (text/image/PDF/Office…) with a draggable half-screen ratio and full-preview mode — globally trusted, no sidebar tab created
